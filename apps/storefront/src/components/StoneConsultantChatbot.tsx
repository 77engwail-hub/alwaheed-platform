...
export const StoneConsultantChatbot: React.FC = () => {
  ...
  const handleSendMessage = (textToSend?: string) => {
    ...
    setTimeout(() => {
      ...
      } else if (lower.includes('نحت') || lower.includes('تاج') || lower.includes('تيجان') || lower.includes('عمود') || lower.includes('أعمدة') || lower.includes('قوس') || lower.includes('مشربية') || lower.includes('cnc') || lower.includes('مخارط')) {
        botResponse = {
          id: (Date.now() + 1).toString(),
          sender: 'bot',
          text: 'نفتخر بأحدث أسطول من مكائن النحت الآلية CNC، المخارط الحديثة، وأجهزة القص الرقمية فائقة الدقة 🏛️\nنقوم بنحت وتشكيل التيجان الكورنثية والإسلامية، الأعمدة المبرومة والمضلعة، المشربيات المفرغة، وإطارات الشبابيك المقوسة بأعلى درجات الدقة الهندسية.',
          timestamp: 'الآن',
          cta: {
            type: 'products',
