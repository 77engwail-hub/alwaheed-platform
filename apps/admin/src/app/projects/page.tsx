  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-extrabold text-stone-900">إدارة سابقة الأعمال والمشاريع</h1>
          <p className="text-xs text-stone-500 mt-0.5">إضافة وتوثيق الواجهات والمشاريع المنفذة ومعارض الصور</p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {projects.map((prj) => (
              <div
                key={prj.id}
                className="bg-stone-50 rounded-2xl overflow-hidden border border-stone-200 space-y-3 p-3.5 sm:p-4 flex flex-col justify-between"
              >
        ...
        <div className="fixed inset-0 z-50 bg-stone-950/75 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto space-y-5 sm:space-y-6 shadow-2xl">
            <div className="flex justify-between items-center pb-3 border-b border-stone-100">
              <h2 className="text-base sm:text-lg font-bold text-stone-900">إضافة مشروع وسابقة أعمال جديدة</h2>
